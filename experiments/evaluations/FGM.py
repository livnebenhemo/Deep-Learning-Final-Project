...
def evaluate_fgm_two_images(self, epsilon, is_defense=False):
    ...
    for data_batch in tqdm(self.dataloader):
        ...
        with torch.no_grad():
            ...
            # Call FGM Attack - pertubate 1 or 2 images ????
            perturbed_data1 = fgm_attack(data1, epsilon, data1_grad)
            perturbed_data2 = fgm_attack(data2, epsilon, data2_grad)
    ...
    try:
        with open("fgm_" + str(epsilon) + "_" + experiment_cfg.output.res_txt_fname, 'w') as f:
...
def evaluate_fgm_one_image(self, epsilon, is_show=False, is_defense=False):
    q_est_all, t_est_all = [], []
    adv_examples = []
    print(f'Evaluate on the dataset...')
    ...
    try:
        with open("one_image_fgm_" + str(epsilon) + "_" + experiment_cfg.output.res_txt_fname, 'w') as f:
            for q_est, t_est in zip(q_est_all, t_est_all):
                f.write(f"{q_est[0]} {q_est[1]} {q_est[2]} {q_est[3]} {t_est[0]} {t_est[1]} {t_est[2]}\n")
    except:
        with open(experiment_cfg.output.res_txt_fname, 'w') as f:
