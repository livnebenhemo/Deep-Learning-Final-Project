...
class SevenScenesTestDataset(object):
    ...
    def _read_pairs_txt(self):
        ...
        with open(pairs_txt, 'r') as f:
            for line in f:
                ...
                # TODO: check this !!!! Is this the truth ???
                t_gt.append(torch.FloatTensor([float(chunks[5]), float(chunks[6]), float(chunks[7])]))
                q_gt.append(torch.FloatTensor([float(chunks[8]),
                                               float(chunks[9]),
                                               float(chunks[10]),
                                               float(chunks[11])]))
                if count == 19999:
    ...
    def __getitem__(self, item):
        img1 = Image.open(self.fnames1[item]).convert('RGB')
        img2 = Image.open(self.fnames2[item]).convert('RGB')
        """img1 = self.fnames1[item]
        img2 = self.fnames2[item]"""
        t_gt = self.t_gt[item]
        q_gt = self.q_gt[item]
