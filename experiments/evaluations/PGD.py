...
def evaluate_pgd_one_image(self, epsilon, is_show=False, is_defense=False, is_distance=False):
    ...
    for data_batch in tqdm(self.dataloader):
        ...
        data1 = data1 + torch.FloatTensor(data1.shape).uniform_(-epsilon, epsilon)
        for j in range(iterations_number):
            ...
            with torch.no_grad():
                ...
                # Call I-FGSM Attack - pertubate 1 or 2 images ????
                data1 = pgd_attack(original_data1, data1, alpha, epsilon, data1_grad)

        with torch.no_grad():
            if is_show and i % 200 == 0:
            ...
            q_est, t_est = self.model(data1, data2)
            if is_defense:
                q_est, t_est = self.model(self.eval_augmentation(data1), data2)

            if is_distance:
                distance += nn.MSELoss(reduction='mean')(original_data1, data1)
    ...
    try:
        with open("one_image_pgd_" + str(epsilon) + "_" + experiment_cfg.output.res_txt_fname, 'w') as f:
...
def evaluate_pgd_two_images(self, epsilon, is_show=False, is_defense=False, is_distance=False):
    ...
    for data_batch in tqdm(self.dataloader):
        ...
        data1 = data1 + torch.FloatTensor(data1.shape).uniform_(-epsilon, epsilon)
        data2 = data2 + torch.FloatTensor(data2.shape).uniform_(-epsilon, epsilon)
        for j in range(iterations_number):
            # Set requires_grad attribute of tensor. Important for Attack
            for parameter in self.model.parameters():
                parameter.requires_grad = False
            ...
            with torch.no_grad():
                ...
                # Call I-FGSM Attack - pertubate 1 or 2 images ????
                data1 = pgd_attack(original_data1, data1, alpha, epsilon, data1_grad)
                data2 = pgd_attack(original_data2, data2, alpha, epsilon, data2_grad)
    ...
    try:
        with open("two_images_pgd_" + str(epsilon) + "_" + experiment_cfg.output.res_txt_fname, 'w') as f:
            for q_est, t_est in zip(q_est_all, t_est_all):
                f.write(f"{q_est[0]} {q_est[1]} {q_est[2]} {q_est[3]} {t_est[0]} {t_est[1]} {t_est[2]}\n")
    except:
        with open(experiment_cfg.output.res_txt_fname, 'w') as f:
