    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for data_batch in tqdm(self.dataloader):
                ...
                """img1 = Image.open(data_batch['img1']).convert('RGB')
                img2 = Image.open(data_batch['img2']).convert('RGB')

                q_est, t_est = self.model(img1.to(self.device),img2.to(self.device))"""
